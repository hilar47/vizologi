					...
					for (var j = 0; j < the_props.length; j++)
					{
						...
						if (the_prop)
						{
							...
							if (the_match)
							{
								...
								//Apply Text editor to text area which is created
								if($(elem).is("textarea")){
									if($(elem).attr('data-apply_editor')){
										if($(elem).data("apply_editor")==true){
											var id = $(elem).attr("id");
											var splited_id = id.split("_");
											var element_id = splited_id[0]+"_"+parseInt(parseInt(the_match[1])+parseInt(1))
											$(elem).attr("id", element_id);
											add_editor(element_id);
										}
									}
								}
							}
